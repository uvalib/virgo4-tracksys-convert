...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="externalSSIDfile" select="'vickery/vickery-SSIDs.xml'"/>

  <xsl:param name="defaultRightsText">Attribution-NonCommercial 2.0 Generic (CC BY-NC
    2.0)</xsl:param>

  <xsl:param name="defaultRightsURI">
    <xsl:text>https://creativecommons.org/licenses/by-nc/2.0/legalcode</xsl:text>
  </xsl:param>

  <!-- program name -->
  <xsl:variable name="progName">
    <xsl:text>vickery2uvaMAP.xsl</xsl:text>
  </xsl:variable>
        ...
        <doc>
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              ...
              <!--<xsl:variable name="photographer">
                <xsl:value-of select="normalize-space(replace(normalize-space(*:Photographer), '\([^\)]*\)', ''))"/>
              </xsl:variable>-->
              <xsl:analyze-string select="*:Title" regex="\|">
                <xsl:non-matching-substring>
                  <xsl:variable name="fieldName">
              ...
              <xsl:apply-templates select="*:Date | *:Work_Date"/>
              <xsl:apply-templates select="*:Current_Site | *:Location | *:Spatial_coordinates"/>
              <xsl:apply-templates
          ...
          <!-- <field name="useRestrict" valueURI="http://rightsstatements.org/vocab/InC/1.0/">In
            copyright</field>-->

          <!-- Subject terms -->
          <xsl:apply-templates
            select="*:Artstor_Classification | *:Category | *:Image_Subject | *:Work_Subject"/>
  ...
  <xsl:template match="*:Country | *:Current_Site | *:Location">
  ...
  <xsl:template match="*:Category | *:Image_Subject | *:Work_Subject">
    <xsl:analyze-string select="." regex="(;|\|)">
      <xsl:non-matching-substring>
        <xsl:if test="normalize-space(.) ne ''">
          <field name="subject" authority="AAT">
