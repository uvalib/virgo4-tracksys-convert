...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:variable name="progname">
    <xsl:text>updateMedicalArtifacts.xsl</xsl:text>
  </xsl:variable>
  ...
  <xsl:template match="*:mods">
    <mods xmlns="http://www.loc.gov/mods/v3">
      ...
      <xsl:choose>
        <xsl:when
          test="*:relatedItem[@type = 'host']/*:titleInfo/*:title[matches(., 'Medical Artifacts Collection')]">
  ...
  <xsl:template match="*:relatedItem[@type = 'original']">
    <relatedItem xmlns="http://www.loc.gov/mods/v3">
      ...
      <xsl:if
        test="../*:relatedItem[@type = 'host']/*:titleInfo/*:title[matches(., 'Medical Artifacts Collection')]">
        <xsl:apply-templates select="../*[matches(local-name(), '(name|genre|abstract)')]"/>
      </xsl:if>
      <xsl:apply-templates/>
    </relatedItem>
  </xsl:template>
